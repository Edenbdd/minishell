...
{
     printf("\033[34mcoucou je suis dans fill cmd expand - exloop_env_var au debut -- le type est : %d -- la value est : %s\033[0m\n", current_token->type, current_token->value);
	if (!expand(manager, current_token, s_env))
	{
     printf("\033[34mcoucou je suis dans fill cmd expand - exloop_env_var IF\033[0m\n");
        current_token->value = NULL;
    ...
    else
    {
		...
		current_token->type = is_operators_wo_expand(manager, current_token->value, 0);
printf("\033[34dans fill cmd expand - exloop_env_var - ELSE le type est : %d\033[0m\n", current_token->type);
printf("\033[34dans fill cmd expand - exloop_env_var - ELSE la value est : %s\033[0m\n", current_token->value);
...
int expand_loop(t_token *current_token, t_env *s_env, t_manager *manager)
{
    while (current_token && current_token->type != PIPE)
    {
        if (current_token->type == ENV_VAR)
        {
 printf("\033[34mcoucou je suis dans fill cmd expand - expand-loop et type = %d\033[0m\n", current_token->type);
 printf("\033[34le type est : %d -- la value est : %s\033[0m\n", current_token->type, current_token->value);
